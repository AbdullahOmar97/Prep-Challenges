...
const cvsFiltration = (arr) => {
    ...
    return arr.filter(candidate => candidate.yearsOfExperience > 4 && candidate.tech === "JS")

        .map(candidate => {

            const fullName = (candidate.firstName && candidate.LastName) ? `${candidate.firstName} ${candidate.LastName}` : candidate.firstName || candidate.LastName || '';
