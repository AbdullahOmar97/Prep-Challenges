  ...
  let recipeInfoArr = [];
    ...
    for (const [key, value] of Object.entries(obj)) {
        if (Array.isArray(value)) {
          recipeInfoArr.push(`${key}: ${value.join(',')}`);
        } else {
            recipeInfoArr.push(`${key}: ${value}`);
        }
    }

    return recipeInfoArr;
